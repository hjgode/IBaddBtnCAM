...
//struct for message queue messages
typedef struct {
	HWND hWnd;
	UINT msg;
	WPARAM wParm;
...
static HANDLE  hExitEvent = NULL;						//event used to end threads
static HANDLE  hThread = NULL;							//handle for thread
static HWND    hWndWatch = NULL;						//handle for watch thread
static WNDPROC RDMWndFunc = 0;							//var to hold old wndproc if subclassing

const TCHAR szTextFileName[] = L"\\IBinjectDll.txt";	//log file name
